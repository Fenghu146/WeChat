...
| **E** | 测试与界面工程 | 自包含断言框架 + 10 个 ctest 回归套件（115 用例）、手动测试工作台（数字键 / 表单 / 分屏 / 失败原因提示） |
...
tests/              10 个回归套件（共 115 个用例）+ 自包含断言框架 fh_mini_test.hpp
...
```bash
cmake --build build --config Debug
ctest --test-dir build -C Debug --output-on-failure     # 预期：10/10 全部通过
```

十个回归套件（共 **115 个用例，全部通过**）与阶段对应：
...
| `boundary_robustness_test` | **边界条件与鲁棒性专项**：消息长度精确边界、5 类型 × 3 平台能力矩阵、回复能力差异、聊天记录 50/51 条淘汰、群 49 人满员拒绝与名额释放、撤回窗 0 秒/运行期收紧、讨论组容量与解散规则、好友/推荐前置条件矩阵、年龄与绑定边界、开通/登录/简单确认联动、空/损坏存档与特殊字符、坏 M 行跳过 | A/B/C/D |
...
| `reports/测试报告.md` | 测试报告：10 套件 115 用例逐项结果、需求覆盖度与代码质量检验 |
