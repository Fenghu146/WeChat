...
| **E** | 测试与界面工程 | 自包含断言框架 + 11 个 ctest 回归套件（123 用例，含边界/鲁棒性专项与真实二进制全流程冒烟）、手动测试工作台（数字键 / 表单 / 分屏 / 失败原因提示） |
...
> 完整的手动走查清单、工作台按键地图与逐项预期结果，见 `reports/演示与手动验证指南.md`。

进入工作台后先选择演示账号；账号列表末尾的「操作说明」（主界面为「测试指引」）可查看操作指引与建议测试路径。
**所有操作项均为数字键**（`0` = 返回上级，触发无需回车；账号选择、群大厅
等列表型界面的功能键按列表长度动态编号）；需要输入文本时按提示输入、
直接回车可取消；每次操作后界面自动重绘，提示条显示成功或失败原因。
正式群会话内按 `0` 直接返回会话列表，按 `8` 进入「更多操作」：切换管理
模式 / 转让群主 / 解散群 / 退出本群 / 群设置（邀请开关、撤回时间窗）/
群公告 / 改群名 / 以其他成员身份操作。
官方/自建群会话（官方群大厅）：`0` 返回；微信群额外提供 `4` 推荐好友入群
（微信群只能由群内成员推荐加入）；自建群群主额外提供 `5` 解散本群、
`6` 转让群主，群主须先转让或解散方能退群。
...
```bash
cmake --build build --config Debug
ctest --test-dir build -C Debug --output-on-failure     # 预期：11/11 全部通过
```

十一个回归套件（共 **123 个用例，全部通过**）与阶段对应：

| 套件 | 覆盖 | 对应阶段 |
|---|---|---|
| `group_core_test` | 领域实体校验、聚合根 11 种操作、撤回时间窗、转让/解散、切换模式 | A |
| `abstract_policy_test` | 六步授权链、成员关系、禁言状态、撤回归属与窗口边界、平台差异对管理动作的收窄 | A |
| `qq_wechat_policy_test` | QQ/微信差异矩阵（邀请、全员禁言、踢人/禁言、改群名/发公告、发言） | A |
...
| `boundary_robustness_test` | **边界与鲁棒性专项**：查询结果按值快照（建群/解散后仍有效）、空/重复消息标识、全空白群名与公告、非法撤回窗口、单员禁言对管理员的实际效果、人数上限“恰好/超一”、群主与管理员不变量、越界消息类型、文本长度上下边界、加群/推荐/踢人权限与满员、建群参数边界、预置群注入幂等、存档往返（特殊字符/号码含逗号/CRLF）、空与损坏存档不覆盖数据、非法数值与重复群号、好友存档损坏、开通情况按存档重建、写盘失败不破坏原存档 | A/B/C/D |
| `demo_flow_smoke_test` | **真实二进制全流程冒烟**：执行 `demo_fh --demo` 跑完 A→D 自动演示，校验自检汇总“全部符合预期”、平台差异场景确实被拦截，并验证 stdin 结束时工作台正常退出（无死循环） | A/B/C/D |
...
| 踢人 / 禁言成员 | ADMIN+ 可执行（不能操作同级或群主） | 仅群主可执行 |
| 改群名 / 发群公告 | ADMIN+ 可执行 | 仅群主可执行 |
| 变更群配置（撤回时间窗、成员邀请开关） | ADMIN+ 可执行 | 仅群主可执行 |
| 设置全员禁言 | ADMIN+ 可执行 | 仅群主可执行 |
| 全员禁言中普通成员发言 | 失败 | 失败 |
| 全员禁言中 ADMIN/群主发言 | 成功 | 成功 |

> 任务书 3.(3)：QQ 群有以群主为核心的管理员制度，微信群仅有群主为
> 特权账号。微信群中管理员角色不产生任何特权（邀请、禁言、踢人、改群名、
> 发公告均仅群主）；从 QQ 群动态切换为微信群时成员数据不受伤，权限按当前
> 策略重新解释。
...
- 入群规则（任务书 3.(3)）：**QQ 群可申请加入**（`joinGroup`），
  **微信群只能推荐加入**（直接申请被拒，须群内成员经
  `inviteIntoGroup` 推荐好友进入）；工作台官方群会话里，微信群提供
  `4 推荐好友入群` 入口（被推荐者须已绑定微信、不在群内且群未满员）；
- 预置微信群的初始成员：库层 `seedPredefined` 默认建空群，演示环境在
  启动加载存档后由 `ensurePredefinedMembers` 为**空成员**的预置群注入
  演示账号（1003 家庭群、1004 同事群），因此预置微信群不再“无人可推荐”
  而成为死群；该注入仅对空群生效，用户手动退群后重启不会被强行加回；
- 群管理（任务书 3.(2)）：加入、退出、**挨踢**（`kickMember`：QQ 群按
  群主>管理员>普通成员分级授权；微信群仅群主）、查询群成员
  （`memberIdsOf`；查询接口一律【按值返回快照】，不暴露内部容器指针，见 `boundary_robustness_test` 的 QueryResultsAreSnapshotsNotInternalPointers）；QQ 群管理员由群主任命（`setGroupAdmin`），
  微信群无管理员制度；自建群群主另有 `disbandGroup`（解散，从群目录移除）
  与 `transferOwner`（转让），且**群主不能直接退群**——与聚合根
  `GroupFH` 口径一致；
...
| 文档 | 内容 |
|---|---|
| `reports/实验报告.md` | 课程实验报告（任务分析 / 方案 / 详细设计 / 总结与最新进展） |
| `reports/项目设计文档(C++).md` | 群组管理部分的设计文档（权限矩阵、时序、模板方法说明） |
| `reports/实现计划与验收清单.md` | 模块划分、实现顺序、接口约定与验收清单 |
| `reports/类图.md` | 全量类图（Mermaid，含关键时序） |
| `reports/测试报告.md` | 测试报告：11 套件 123 用例（含边界/鲁棒性专项与全流程冒烟）逐项结果、需求覆盖度与代码质量检验 |
| `reports/设计验证报告.md` | 设计步骤回归检查报告（对照任务书"三、设计步骤"四步逐条核对） |
| `reports/任务书逐条对齐.md` | 对照任务书逐条核对：题目描述 1~6、技术层次 1~5、设计步骤 1~4 的实现与测试证据 |
| `reports/演示与手动验证指南.md` | 演示与手动验证操作手册：构建/测试/分段演示命令、工作台按键地图、逐项走查清单 |
...
- 手动工作台与 `--demo` 自动演示共用一个进程内“数据世界”，多次操作数据持续累积，
  ...
  即恢复初始状态）；
- 预置微信群的初始成员为**演示注入**：每次启动对「空成员」的预置群补齐，
  因此把 1003/1004 清空后重启会自动恢复成员；如需保留“空群”状态请勿依赖重启；
