...
| **E** | 测试与界面工程 | 自包含断言框架 + 11 个 ctest 回归套件（122 用例，含边界/鲁棒性专项与真实二进制全流程冒烟）、手动测试工作台（数字键 / 表单 / 分屏 / 失败原因提示） |
...
tests/              11 个回归套件（共 122 个用例）+ 自包含断言框架 fh_mini_test.hpp
...
| `boundary_robustness_test` | **边界与鲁棒性专项**：空/重复消息标识、全空白群名与公告、非法撤回窗口、单员禁言对管理员的实际效果、人数上限“恰好/超一”、群主与管理员不变量、越界消息类型、文本长度上下边界、加群/推荐/踢人权限与满员、建群参数边界、预置群注入幂等、存档往返（特殊字符/号码含逗号/CRLF）、空与损坏存档不覆盖数据、非法数值与重复群号、好友存档损坏、开通情况按存档重建、写盘失败不破坏原存档 | A/B/C/D |
