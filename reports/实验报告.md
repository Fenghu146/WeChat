...
app/            main.cpp（入口）+ demo_runner.cpp（--demo 自动演示）+ client_ui.cpp（手动测试工作台）+ ui_screen.cpp（终端排版与整帧重绘）
...
- **排版与重绘**：所有界面由 `fh_ui::Screen`（`app/ui_screen.hpp/.cpp`，见 5.6）整帧缓冲后统一重绘 —— 输出行数受终端可视行数约束、每行宽度受可视列数约束，因此任意窗口尺寸下都不会把标题顶出屏幕；超出一屏的列表项折叠为「…… 还有 N 条」，长帮助页自动分页。中文按显示宽度对齐（全角与中文环境下的歧义宽度字符按 2 列计），键值卡片与菜单列因此能整齐对齐。
...
- 在 `reports/` 下集中存放 8 份项目文档：实验报告、项目设计文档、实现计划与验收清单、类图、测试报告、设计验证报告、任务书逐条对齐、演示与手动验证指南。
...
| # | 位置 | 改动 | 收益 |
|---|---|---|---|
| 1 | `client_ui.cpp::busy()` | 取消每次操作固定 `sleep(45ms)×5 ≈ 225ms` 的人为延迟，默认瞬时返回；需观察动画时设 `FH_UI_ANIM=1` | 每次业务操作响应时间 −225ms |
| 2 | `client_ui.cpp::cls()` | `std::system("cls")` 改为 `FillConsoleOutputCharacter/Attribute` + `SetConsoleCursorPosition` | 每次界面重绘不再创建 `cmd.exe` 子进程 |
...
| 6 | `client_ui.cpp` / `main.cpp` | `windows.h` 前定义 `NOMINMAX` | 消除 `min/max` 宏破坏标准库的隐患 |
| 7 | `client_ui.cpp` | 删除未被引用的 `accountNo()` | 去掉死代码与 1 条编译告警 |
...
**有意未做（避免无谓改动）**：`findGroup` / `findEdge` / `findMessage` 等 O(n) 线性查找在演示数据规模下不构成瓶颈，待规模放大后再引入索引；`client_ui.cpp` 中局部 `g` 遮蔽全局 `App g` 属可读性问题，暂缓处理。
...
**修复**：新增 `app/ui_screen.hpp / .cpp`（命名空间 `fh_ui`），把排版能力集中到一处，工作台与自动演示共用。
...
**关键设计取舍**：成员注入放在**应用层**（`client_ui.cpp` 启动时回填）而非库层 `seedPredefined`，因此库层默认行为不变 —— 现有 9 套件 98 用例零改动，测试对「预置群默认无成员」的断言继续成立。回填在 `loadWorldFromDisk()` **之后**执行（存档载入会整体替换群目录），故已有存档中的空群同样会被补齐；且仅对空群生效，用户手动退群后重启不会被强行加回。
