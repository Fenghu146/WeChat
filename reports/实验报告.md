...
tests/          10 个回归套件（115 个用例）+ 自包含断言框架 fh_mini_test.hpp
```
...
ctest --test-dir build -C Debug --output-on-failure   # 10/10 套件、115 用例全部通过
```
...
| `boundary_robustness_test` | 边界条件与鲁棒性专项（消息长度/类型矩阵、容量满员、时间窗、损坏存档、特殊字符转义） |
...
7. **可验证性强**：115 个用例覆盖任务书逐条要求，另有手动测试工作台支持全流程走查。
