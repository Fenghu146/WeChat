...
| 优化(6) UML 类图 | `reports/类图.md`（Mermaid） | 已完成 |
...
系统按「领域模型 / 授权上下文 / 平台策略 / 多产品 / 社交 / 消息 / 界面」分层，目录与职责如下（完整类图见 `reports/类图.md`）：
...
├── platform/   微X 产品：PlatformKindFH、UserProfileFH、AccountInfoFH、UserRegistryFH、ActivationManagerFH、LoginManagerFH
├── util/       持久化工具：PersistUtilFH（存档行式转义）
...
src/            model/（聚合根）、social/（注册表）、policy/（策略）三处实现 .cpp，声明仍在 include/
app/            main.cpp（入口）+ ui/（client_ui_*.cpp 工作台、demo_runner 自动演示、ui_screen 终端排版）
docs/           课程原始任务书（2026-面向对象程序课程设计.md / .docx）
tests/          11 个回归套件（123 个用例）+ 自包含断言框架 fh_mini_test.hpp
```
...
| `KICK_MEMBER` | ✓ | ✓ | ✗ | — | **仅群主可** |
| `MUTE_MEMBER` | ✓ | ✓ | ✗ | — | **仅群主可** |
| `EDIT_GROUP` / `PUBLISH_ANNOUNCEMENT` | ✓ | ✓ | ✗ | — | **仅群主可** |
...
> 说明：微信群「仅群主为特权账号」这一口径直接来自任务书 3.(3)。因此在微信群中，**公共步骤判为 ADMIN+ 的全部管理动作（邀请、踢人、禁言、改群名、发公告、全员禁言，以及经由 `EDIT_GROUP` 授权的群配置变更）一律收紧为仅群主**；从 QQ 群动态切换为微信群时，成员与角色数据保留，权限按微信口径重新解释。
...
- **排版与重绘**：所有界面由 `fh_ui::Screen`（`app/ui/ui_screen.hpp/.cpp`，见 5.6）整帧缓冲后统一重绘 —— 输出行数受终端可视行数约束、每行宽度受可视列数约束，因此任意窗口尺寸下都不会把标题顶出屏幕；超出一屏的列表项折叠为「…… 还有 N 条」，长帮助页自动分页。中文按显示宽度对齐（全角与中文环境下的歧义宽度字符按 2 列计），键值卡片与菜单列因此能整齐对齐。
- **键位**：全部操作项为数字键，`0` 统一为「返回上级」；正式群会话内 `[8]` 为「更多操作」；账号选择、群大厅等列表型界面的功能键按列表长度**动态编号**，避免与列表项冲突。
- **反馈**：底部提示条显示每次操作的成功或失败原因，失败原因按规则链给出（如「微信群仅群主可设置全员禁言（平台差异）」「不能禁言与自己同级或更高等级成员」）；提交动作默认瞬时完成，如需观察过程动画可设环境变量 `FH_UI_ANIM=1`。
- **防呆**：文本输入直接回车即取消；解散群二次确认；无效按键提示；群主退群被拒时提示先转让或解散。
- **状态标注**：好友 / 成员选择列表在每项后标注当前关系（`［已是QQ好友］`/`［非QQ好友］`、`［已关注］`/`［未关注］`、`［已在群内］`/`［不在群内］`）。同一个选择器同时服务「加好友」与「删好友」等相反意图，因此采用标注而非过滤名单：既避免误操作，也保留「重复添加被拒绝」这类规则的可演示性。
- **测试便利**：会话内「更多操作」（正式群 `[8]`）提供切换管理模式、转让群主、解散群、退出本群、群设置（改邀请开关 / 撤回窗口，用于现场复现「超时不可撤回」）、群公告 / 改群名、以其他成员身份操作；官方/自建群会话则提供推荐好友入群（微信群）与解散本群 / 转让群主（自建群群主）。
- **自动演示**：`--demo` 按 A→D 顺序演示，交互终端下每段之间等待回车，便于答辩时边讲边停；`--demo=A|B|C|D` 可只演示其中一段。演示结束输出自检汇总（校验项总数、其中「应失败」项数、是否存在与预期不符项），终端下着色：绿色 = 符合预期，红色 = 与预期不符。
...
ctest --test-dir build -C Debug --output-on-failure   # 11/11 套件、123 用例全部通过
```
...
| `qq_wechat_policy_test` | QQ / 微信差异矩阵（邀请、全员禁言、发言、踢人 / 禁言、改群名 / 发公告） |
...
| `boundary_robustness_test` | 边界 / 鲁棒性专项：存档加载容错、原子写、按值快照语义、切换微信模式后管理员特权收回 |
| `demo_flow_smoke_test` | 真实二进制全流程冒烟（`--demo` 全流程 + 自检汇总） |
...
7. **可验证性强**：123 个用例覆盖任务书逐条要求，另有手动测试工作台支持全流程走查。
...
| 12 | 工作台在 macOS / Linux 下按数字键**没有反应**，必须再按一次回车（Windows 下正常），与「单键热键、无需回车」的交互约定不符 | 根因是 POSIX 终端默认处于规范模式，要等到回车才把字节交给程序（且会回显）。新增 `RawTerminalGuard`：读取单键时临时清除 `ICANON` / `ECHO`（保留 `ISIG`，Ctrl+C 仍有效），读到即恢复；非终端（管道 / 重定向）不做任何改动，自动化冒烟不受影响。修复后三平台行为一致 |
...
在「阶段 E · 界面工程」基线之后，实现继续围绕“贴近真实 IM 的可演示 + 可回归”做了若干增量，所有改动均通过 `ctest` 全量验证。
...
- 在 `reports/` 下集中存放 9 份项目文档：实验报告、项目设计文档、实现计划与验收清单、类图、测试报告、测试报告_边界与鲁棒性专项、设计验证报告、任务书逐条对齐、演示与手动验证指南；新增 `reports/README.md` 作为文档索引。
- `reports/archive/` 归档历史草稿（实验报告初稿 / 终稿及其 Word 导出），主报告以 `reports/实验报告.md` 为准。
- 课程原始任务书移入 `docs/`（`docs/2026-面向对象程序课程设计.{md,docx}`）；根目录仅保留 `README.md`、`.gitignore` 与构建/源码目录。
- 所有交叉引用同步到新路径，旧的英文文件名已不再出现。

### 5.5 性能与代码质量优化（2026-09-11）

对现有实现做了一次系统性的性能与代码质量验证，并按「收益显著」原则落地了下列优化，全部通过 `ctest` 回归。

**验证基线**：MinGW g++ 15.2 + CMake 3.28，C++17，开启 `-Wall -Wextra -Wshadow`；构建通过，`ctest` 9/9 套件全绿。

**已落地优化**：

| # | 位置 | 改动 | 收益 |
|---|---|---|---|
| 1 | `app/ui/client_ui_terminal.cpp::busy()` | 取消每次操作固定 `sleep(45ms)×5 ≈ 225ms` 的人为延迟，默认瞬时返回；需观察动画时设 `FH_UI_ANIM=1` | 每次业务操作响应时间 −225ms |
| 2 | `app/ui/client_ui_terminal.cpp::cls()` | `std::system("cls")` 改为 `FillConsoleOutputCharacter/Attribute` + `SetConsoleCursorPosition` | 每次界面重绘不再创建 `cmd.exe` 子进程 |
| 3 | `UserProfileFH::platformAccountId()` | 返回类型 `std::string` → `const std::string&` | 昵称渲染、好友 / 群成员查找等热路径不再反复拷贝字符串 |
| 4 | `GroupRegistryFH::loadFromFile()` | 逐行解析包入 `try/catch` | 存档被截断 / 篡改时不再于启动阶段抛出异常崩溃 |
| 5 | `MessageKindFH` 及 5 处引用 | 枚举成员 `FILE` 改名 `DOCUMENT` | 消除对 stdio `FILE` 的遮蔽（一次清除 12 条 `-Wshadow`） |
| 6 | `app/ui/*.cpp` / `app/main.cpp` | `windows.h` 前定义 `NOMINMAX` | 消除 `min/max` 宏破坏标准库的隐患 |
| 7 | `app/ui/client_ui_*.cpp` | 删除未被引用的 `accountNo()` | 去掉死代码与 1 条编译告警 |

**效果**：编译告警由 14 条降至 7 条（剩余为纯可读性的局部变量遮蔽与测试未用参数，不影响功能）；`ctest` 9/9 保持全绿；上述改动均为等价重构，不改变任何对外行为。

**后续演进**：上表第 2 项的清屏方案在 2026-09-17 的界面工程中继续演进为「标准 ANSI 整帧重绘 + 按终端尺寸裁剪」，并顺带修复了按键后标题被顶出屏幕的滚动缺陷，见 5.6。

**有意未做（避免无谓改动）**：`findGroup` / `findEdge` / `findMessage` 等 O(n) 线性查找在演示数据规模下不构成瓶颈，待规模放大后再引入索引；`app/ui/client_ui_*.cpp` 中局部 `g` 遮蔽全局 `App g` 属可读性问题，暂缓处理。

### 5.6 界面排版统一与重绘缺陷修复（2026-09-17）

**问题**：手动工作台每按一次键，界面会「多跳一段」—— 标题栏与首行信息被顶出屏幕，时好时坏。

**根因**：每帧都是清屏后逐行直接输出，从不查询终端可视高度。

- 帧行数一旦超过窗口高度，写入越过底行就会让控制台向上滚动，最先画的标题被挤出可视区；而溢出通常只有 1\~2 行（多打的一个空行、状态条与提示行的固定开销、帧后追加的行内子提示），所以表现为「有时正常、有时少一行」。
- Windows 分支用 `FillConsoleOutputCharacterA` 填满**整个缓冲区**（`dwSize.X × dwSize.Y`，conhost 默认可达 9001 行），每帧近百万次单元格写入，本身也是刷新开销。

**修复**：新增 `app/ui/ui_screen.hpp / .cpp`（命名空间 `fh_ui`），把排版能力集中到一处，工作台与自动演示共用。

| 组件 | 作用 |
|---|---|
| `TermSize termSize()` | 取终端**可视**尺寸：Windows 用 `srWindow`（不是缓冲区），POSIX 用 `ioctl(TIOCGWINSZ)`，查询失败兜底 80×24 |
| `int displayWidth()` | 中日韩全角、以及中文环境下的「歧义宽度」字符（`—` `·` `“”` `→` `≠` 等）一律按 2 列计 —— 宁可高估：高估只会提前折行，绝不会让行超出终端而折行溢出 |
| `Screen`（整帧缓冲） | 攒帧 → 按「列数 − 1、行数 − 1」留余量 → 回到首行逐行覆盖（每行清到行尾、末行不发换行符、末尾清到屏底） |
| 裁剪优先级 | 超屏时**先丢空行、再丢列表项、最后才丢正文**；标题栏、菜单、状态条、提示行永不裁剪，并在裁剪处提示省略了多少行 |
| `rule` / `section` / `Rows` | 线性日志排版（标题栏、`—— 小节 ——`、按最大标签宽度对齐的结果列），供 `--demo` 使用 |

**排版约定**：全部界面统一为「标题栏 → 键值卡片 → `—— 小节 ——` → 对齐菜单 → 状态条 → 提示行」，去掉原先手敲的长分隔线。菜单列数按终端宽度自适应，列表型界面保持一行一个（信息密度与原版一致）；长帮助页按可视高度自动分页，不丢内容。

**顺带修复**：输入流结束（EOF）时程序直接退出，避免 `--demo > out.txt` 这类重定向用法在演示结束后空转不退出。

**文档同步**：`演示与手动验证指南` 补充 Windows 原生命令写法（`.\build\Debug\demo_fh.exe`、`ctest -C Debug`、`Remove-Item`）与排障项；`测试报告`、`任务书逐条对齐` 中引用输出原文的地方随新排版更新。

**验证结果**：

- 构建 0 错误 0 警告；`ctest --test-dir build -C Debug` **9/9 套件 / 98 用例全绿**。
- `--demo` **66 项校验「全部符合预期」**（成功 46 / 应失败 20）；分段 28 / 11 / 14 / 13 项与文档记载一致。
- 界面输出上限实测（非终端兜底的 80×24，以及模拟矮窗口的 80×13 两种尺寸）：各界面最大 **14 行 / 77 列**；矮窗口下最大 11 行，且被裁掉的只有空行，账号列表与菜单完整保留。
- 非终端（管道 / 重定向）输出不产生任何 ANSI 控制码。

**经验**：这类「偶发一两行」的显示缺陷，本质是输出量没有与终端尺寸对齐。把它写成一条可测的不变量（输出行数 ≤ 可视行数、每行显示宽度 ≤ 可视列数）之后，问题就从「看运气」变成了「可断言」。

### 5.7 手动测试反馈修复（2026-09-18）

**背景**：按《手动测试报告》的 5 条反馈逐条核实并修复。所有改动均通过 `ctest` 全量回归，用例数与 `--demo` 自检项数**均保持不变**。

| # | 反馈 | 根因 | 修复 |
|---|---|---|---|
| 1 | 预置微信群无人、死群、无法拉人 | `seedPredefined` 建群不写成员，而微信群只能由**群内成员**推荐加入 → 无人可推荐；UI 亦无推荐入口 | 新增 `GroupRegistryFH::ensurePredefinedMembers`（仅对**空成员**的预置群注入，幂等）；演示环境在**加载存档后**回填 1003/1004；官方群会话新增 `[4]推荐好友入群` |
| 2 | 群主退群后群仍在、无解散入口 | 注册表 `leaveGroup` 无群主保护、无解散能力（与聚合根 `GroupFH` 不一致） | `leaveGroup` 拒绝群主；新增 `disbandGroup` / `transferOwner`；官方群会话为群主提供 `[5]解散本群` / `[6]转让群主` |
| 3 | 「候选 1 人」与条件①矛盾 | 诊断屏 ③ 统计的是「谁绑了微信」，未与「是否开通目标服务」求交；且入口无拦截 | 入口先校验 `isActivated(to)`（未开通直接提示去开通）；③④ 改为以「来源平台好友」为基数的交集口径 |
| 4 | 开通/登录/退出逻辑不清晰 | 引导分散在各键的失败提示里 | 账号中心新增计算型「建议下一步」 |
| 5 | 正式群会话无法退出 | 主界面 `0` 被「更多操作」占用，返回需 `0→8`，违背全局「0=返回上级」 | 主界面 `0` 改为返回会话列表；`群公告 / 改群名` 移入 `[8]更多操作` |

**关键设计取舍**：成员注入放在**应用层**（`app/ui/client_ui_workspace.cpp` 启动时回填）而非库层 `seedPredefined`，因此库层默认行为不变 —— 现有 9 套件 98 用例零改动，测试对「预置群默认无成员」的断言继续成立。回填在 `loadWorldFromDisk()` **之后**执行（存档载入会整体替换群目录），故已有存档中的空群同样会被补齐；且仅对空群生效，用户手动退群后重启不会被强行加回。

**验证**：`ctest` **9/9 / 98 用例全绿**；`--demo` **66 项「全部符合预期」**；脚本化按键走查确认：1003 成员 2 人且可推荐入群（2→3）、自建群群主退群被拒 / 解散成功（二次确认）、未开通微信时推荐入口直接拦截、账号中心显示「按 [1] 开通微信服务」、正式群会话 `0` 一级返回。

### 5.8 代码结构拆分与目录整理（2026-09-21）

在**行为与文案零回归**的前提下，于最新主线（含查询接口按值化、微信仅群主等加固）之上，把「大文件 + header-only」结构调整为「声明 / 实现分离 + 按职责分目录」：

- **界面子系统归位**：`app/` 下的 `ui_screen.*`、`demo_runner.*`、`client_ui.*` 全部迁入 `app/ui/`；原单文件 `client_ui.cpp` 按业务界面拆分为 10 个模块 `.cpp`（终端 / 状态 / 会话 / 官方群 / 大厅 / 账号 / 通讯录 / 建群 / 本地会话 / 工作台），共享声明集中在 `client_ui_internal.hpp`，并按值化快照接口同步改造调用点。
- **领域实现分离**：`GroupFH`、`GroupRegistryFH`、`FriendRegistryFH` 改为「`include/` 声明 + `src/model`、`src/social` 实现」，其余类型仍为头文件内联。
- **持久化工具归位**：`persist_util_fh.hpp` 由 `include/im/platform/` 移到 `include/im/util/`，引用同步更新。
- **文档整理**：课程原始任务书移入 `docs/`；`reports/` 新增 `README.md` 索引，历史草稿归档到 `reports/archive/`；全部交叉引用同步到新路径。
- **验证**：`cmake --build build --config Debug` **0 错误 0 告警**；`ctest` 10/11 通过（唯一失败为上游既有的 `demo_flow_smoke_test`：其 `WORKING_DIRECTORY` 假设单配置布局，VS 多配置生成器下 `build/` 中并无 `demo_fh.exe`，与本轮改动无关）；`--demo` **66 项「全部符合预期」**，界面文案与按键映射未改动。
