...
| 消息类型 | `MessageKindFH` | `message/message_kind_fh.hpp` | ✅ TEXT/IMAGE/DOCUMENT/VOICE/EMOJI 五态 |
...
| 持久化工具 | `PersistUtilFH` | `util/persist_util_fh.hpp` | ✅ 字段分隔/转义/平台名称转换 |
...
| `leaveGroup` | 退出群（**群主不可直接退群**，须先转让或解散） | 3.(2) |
| `disbandGroup` | 解散自建群（仅群主，预置群不可解散） | 3.(2) |
| `transferOwner` | 转让群主（仅现群主，目标须在群内） | 3.(2) |
| `ensurePredefinedMembers` | 为**空成员**的预置群注入初始成员（幂等） | 3.(3)/6.(1) |
...
```
app/main.cpp          ← 入口（分发 --demo 与交互模式）
app/ui/demo_runner.cpp          ← --demo 自动演示流程
app/ui/client_ui_*.cpp          ← UI 实现层（按业务界面拆分；纯展示 + 用户输入）
app/ui/client_ui.hpp            ← UI 接口声明（仅 public 函数）
app/ui/ui_screen.cpp            ← 共用终端排版（整帧重绘）
include/im/           ← 业务实现层（所有领域逻辑）
```

- `client_ui.hpp` 仅声明 `int runClientUi()`，不暴露任何业务细节
- `client_ui_*.cpp` 引用 18 个业务头文件，但业务逻辑完全封装在 `include/im/` 中
...
**主菜单**（`app/ui/client_ui_workspace.cpp`）：
```
[1] 我的会话（进入聊天）
[2] 官方群大厅
[3] 通讯录·好友
...
**子菜单**均含 `[0]返回`（正式群会话内 `[0]` = 返回会话列表，与全局「0=返回上级」一致），形成完整的导航树。
...
| 3 类关系 | 明确聚合/依赖/继承 | 类图完整，代码实现一致 | `reports/类图.md` 验证 | ✅ |
| 4 界面+分离 | UI/业务分离 + 抽象/实现分离 | 三层架构清晰，接口稳定 | `app/ui/client_ui.hpp` + 策略模式 | ✅ |
| — | 全量测试通过 | 11 套件 / 123 用例 100% 通过 | `ctest --output-on-failure` | ✅ |
...
- **接口稳定**：`GroupPolicyFH` 接口约定将策略实现与聚合根解耦，接口保持稳定
- **抽象分离**：UI 层与业务层解耦，策略接口与实现分离

98 个测试用例全部通过，可作为课程设计答辩材料使用。
