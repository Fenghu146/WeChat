...
> **后续变更（2026-09-21）**：本表记录的是上述五轮实测结果，保持原样。其后新增了
> `boundary_robustness_test`（21 用例）与 `demo_flow_smoke_test`（1 用例），并为
...
> （微信群改群名收窄为仅群主）。**当前为 11 个套件 / 122 个用例全部通过**，
> 明细与前后对比见 `reports/测试报告_边界与鲁棒性专项.md`。
