...
| 第六次（边界/鲁棒性专项与全流程冒烟，macOS） | 2026-09-21 | 11 | 123 | 123 | 0 | ✅ 全部通过 |
| **合计** | — | **11** | **123** | **123** | **0** | **✅ 100%** |
...
> **后续变更（2026-09-21）**：前五轮记录保持原样。其后新增了
> `boundary_robustness_test`（22 用例）与 `demo_flow_smoke_test`（1 用例），并为
> `qq_wechat_policy_test` 补 2 个平台差异用例、把 `abstract_policy_test` 的
> `CommonActionsEqualAcrossPlatforms` 改写为 `SendIsCommonButGroupEditingIsPlatformGated`
> （微信群改群名收窄为仅群主）。第六轮（macOS，2026-09-21）复验 **11 个套件 / 123 个用例
> 全部通过**（ctest 11/11，`--demo` 自检 66 项全部符合预期），明细与前后对比见
> `reports/测试报告_边界与鲁棒性专项.md`。
...
| `SendIsCommonButGroupEditingIsPlatformGated` | 发言两平台一致；改群名属平台差异：QQ 管理员可，微信仅群主 |

### 3.3 qq_wechat_policy_test — 7 用例 ✅
...
| `KickAndMutePlatformDifference` | QQ 管理员可踢/禁言普通成员（不能动同级与群主）vs 微信仅群主 |
| `EditAndAnnouncePlatformDifference` | QQ 管理员可改群名/发公告 vs 微信仅群主 |
...
| `app/ui/client_ui_*.cpp`（原 `app/client_ui.cpp:249`） | 匿名命名空间函数 `accountNo` 定义后未使用（`-Wunused-function`） | 提示 |
